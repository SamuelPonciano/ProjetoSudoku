...
public class Board {
    ...
    public boolean changeValue(final int col, final int raw, final Integer value){
        var space = spaces.get(col).get(raw);
    ...
    public boolean clearValue(final int col, final int raw){
        var space = spaces.get(col).get(raw);
        if(space.isFixed()) return false;
